...
export class WorkspaceDiff {
  ...
  diffContent(oldContent?: string, newContent?: string, filePath = ''): FileDiffItem {
  ...
  generateReport(items: FileDiffItem[]): WorkspaceDiffReport {
    ...
    for (const item of items) {
      if (item.type === 'added') totalAdded++;
      else if (item.type === 'removed') totalRemoved++;
      else if (item.type === 'modified' || item.type === 'renamed') totalModified++;
    }
