...
export function isInCommentOrString(line: string, index: number): boolean {
...
export function classifyUsage(line: string, symbolName: string): ReferenceKind {
  ...
  const defPattern = new RegExp(
    `^(?:export\\s+(?:default\\s+)?)?(?:async\\s+)?(?:function|class|interface|type|enum|(?:const|let|var))\\s+${escaped}\\b`
  );
  ...
  const typePattern = new RegExp(
    `(?::\\s*|\\bextends\\s+|\\bimplements\\s+|\\bas\\s+|<|&\\s*|\\|\\s*)${escaped}\\b`
  );
...
export class ReferenceEngine {
  private cache = new Map<string, ReferenceResult>();
  ...
  findUsages(symbolName: string): ReferenceResult {
    ...
    const definitions: ReferenceLocation[] = [];
    const usages: ReferenceLocation[] = [];
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        while ((match = lineRe.exec(lineText)) !== null) {
          ...
          const isDef = definitions.some(
            (d) => d.filePath === filePath && Math.abs(d.line - lineNum) <= 1
          );
    ...
    const ordered = [...uniqueDefs, ...uniqueUsages].sort((a, b) => {
      if (a.kind === 'definition' && b.kind !== 'definition') return -1;
      if (a.kind !== 'definition' && b.kind === 'definition') return 1;
