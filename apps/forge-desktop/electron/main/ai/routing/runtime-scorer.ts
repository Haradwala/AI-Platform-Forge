...
export class RuntimeScorer {
  scoreCandidates(
    request: ExecutionRequest,
    candidates: RuntimeCandidateInfo[],
    ...
    return candidates
      .map((candidate) => {
        ...
        // 2. Health score (0-20)
        let healthScore = 20;
        if (candidate.health === 'degraded') healthScore = 10;
        if (candidate.health === 'unhealthy' || !candidate.isAvailable) healthScore = 0;

        // 3. Latency score (0-15)
        let latencyScore = 15;
        const latency = candidate.latencyMs ?? 200;
        if (latency > 1500) latencyScore = 2;
        else if (latency > 800) latencyScore = 7;
        else if (latency > 400) latencyScore = 11;
