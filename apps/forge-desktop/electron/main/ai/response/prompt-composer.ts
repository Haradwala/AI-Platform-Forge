...
import type { ResponseRequest, PromptSection, KnowledgeFact } from './response-types';
...
import {
  ...
  GitDiffFormatter,
  ErrorTraceFormatter,
} from './formatters/prompt-formatter-strategy';

export class PromptComposer {
  private readonly registry: PromptFormatterRegistry;

  constructor(registry?: PromptFormatterRegistry) {
    this.registry =
      registry ||
      new PromptFormatterRegistry([
  ...
  compose(request: ResponseRequest): string {
    ...
    if (request.groundedContext?.knowledgeFacts) {
      ...
      if (request.groundedContext?.terminalFacts) {
        facts.push(...request.groundedContext.terminalFacts.map((t) => ({ ...t, kind: 'terminal_output' as const })));
      }
    }

    const groundingContent: string[] = [];
