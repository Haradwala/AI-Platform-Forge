...
export type QueryIntent =
  ...
  | 'count'
  | 'list'
  | 'explain'         // CODE_EXPLAIN — only when no deterministic intent applies
  | 'find_symbol'
  | 'find_references'
  | 'find_imports';
...
export type ExecutionMode =
  | 'deterministic'         // file/folder queries handled without LLM
  | 'engineering-intelligence' // CODE_EXPLAIN — semantic retrieval + EIE
  | 'pipeline';             // general/conversational — full pipeline + LLM
...
export type TargetType =
  ...
  | 'path'       // e.g. "apps/forge-desktop/index.ts"
  | 'concept'    // e.g. "authentication" (for CODE_EXPLAIN)
  | 'symbol';    // e.g. "AuthService" (for symbol queries)
...
function stripConversationalPrefix(prompt: string): string {
 ...
 */
function isValidTarget(candidate: string, intent: QueryIntent): boolean {
  if (!candidate || candidate.length < 2 || candidate.length > 120) return false;

  const lower = candidate.toLowerCase();

  // Reject English filler words
  if (ENGLISH_FILLER_BLOCKLIST.has(lower)) return false;
  ...
  if (isDeterministicIntent) {
    const looksLikeFile =
      candidate.includes('.') ||      // has extension: package.json, tsconfig.ts
...
const OPEN_PATTERNS: RegExp[] = [/\bopen\b/i, /\bview\b/i, /\bread\b/i, /\binspect\b/i];

/** Patterns that signal a FIND intent. */
const FIND_PATTERNS: RegExp[] = [
  /\bfind\b/i,
  /\bsearch\s+(for\s+)?\b/i,
  /\blocate\b/i,
  /\bwhere\s+is\b/i,
...
const FIND_SYMBOL_PATTERNS: RegExp[] = [
  /\b(class|function|interface|type|enum|component|hook)\s+\w+/i,
];
const FIND_REFERENCES_PATTERNS: RegExp[] = [
  /\breferences?\s+(to|of)\b/i,
  /\bwhere\s+is\b.*\bused\b/i,
];
const FIND_IMPORTS_PATTERNS: RegExp[] = [
...
function looksLikeFileDomain(lc: string): boolean {
  return (
    /\bfiles?\b/i.test(lc) ||
...
function looksLikeActiveFileDomain(lc: string): boolean {
 ...
 */
function extractAndValidateTarget(
  originalPrompt: string,
  intent: QueryIntent,
  scope?: string
): { target?: string; targetType?: TargetType; targetValidated: boolean } {
  ...
  const extMatch = originalPrompt.match(
    /\b(\.(?:ts|tsx|js|jsx|json|md|css|scss|html|txt|yaml|yml|env))\b/i
  );
  if (extMatch?.[1]) {
  ...
  if (intent === 'count' || intent === 'list' || intent === 'find') {
    const langMatch = originalPrompt.match(
      /\b(typescript|javascript|json|markdown|css|html|yaml|scss|python|rust|go)\b/i
    );
    if (langMatch?.[1]) {
 ...
 */
function extractScopeToken(prompt: string): string | undefined {
  const m = prompt.match(
    /\b(?:in|under|inside|within)\s+([a-zA-Z0-9_\-][a-zA-Z0-9_\-\/\.]*)/i
  );
...
function extractOrdinal(prompt: string): number | undefined {
  const lc = prompt.toLowerCase();
  for (const [word, pos] of Object.entries(ORDINAL_WORD_MAP)) {
    const re = new RegExp(`\\b${word}\\b`, 'i');
    if (re.test(lc)) return pos;
  }
  return undefined;
}
 ...
 */
function resolveIntent(lc: string, domain: QueryDomain): QueryIntent {
  // 1. Symbol sub-intents (check before generic find)
  if (FIND_REFERENCES_PATTERNS.some((p) => p.test(lc))) return 'find_references';
  if (FIND_IMPORTS_PATTERNS.some((p) => p.test(lc))) return 'find_imports';
  if (domain === 'symbol' && FIND_SYMBOL_PATTERNS.some((p) => p.test(lc))) return 'find_symbol';

  // 2. Deterministic intents — MUST precede 'explain'
  //    "How many package.json files?" → COUNT, never CODE_EXPLAIN
  if (COUNT_PATTERNS.some((p) => p.test(lc))) return 'count';
  if (LIST_PATTERNS.some((p) => p.test(lc))) return 'list';
  if (OPEN_PATTERNS.some((p) => p.test(lc))) return 'open';
  if (FIND_PATTERNS.some((p) => p.test(lc))) return 'find';

  // 3. CODE_EXPLAIN — fallback only, reached after all deterministic paths fail
  if (EXPLAIN_PATTERNS.some((p) => p.test(lc))) return 'explain';
 ...
 */
function deriveExecutionMode(domain: QueryDomain, intent: QueryIntent): ExecutionMode {
  const deterministicIntents: QueryIntent[] = ['open', 'find', 'count', 'list'];
  const deterministicDomains: QueryDomain[] = ['file', 'folder', 'workspace', 'active_file'];
...
export class QueryNormalizationEngine {
  /**
   ...
   */
  static isDeterministic(q: NormalizedQuery, confidenceThreshold = 0.4): boolean {
    // Summarize/explain keyword guard
    const lc = (q.originalPrompt || '').toLowerCase();
    if (lc.includes('summarize')) return false;
