...
exports.WorkflowDefinitionParser = void 0;
class WorkflowDefinitionParser {
     ...
     */
    parse(content, workspaceRoot, format = 'yaml') {
        let parsedObj;
    ...
    validate(def) {
        const errors = [];
        if (!def.id)
            errors.push('Workflow missing required "id" property');
        if (!def.name)
            errors.push('Workflow missing required "name" property');
    ...
    normalizeAndValidate(raw, workspaceRoot, format) {
        ...
        for (const [jobId, rawJob] of Object.entries(rawJobs)) {
            const steps = (rawJob.steps || []).map((step, idx) => ({
...
exports.WorkflowDefinitionParser = WorkflowDefinitionParser;
//# sourceMappingURL=workflow-definition-parser.js.map