...
exports.PlatformInspectorService = void 0;
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
const shared_1 = require("@forge/shared");
const electron_1 = require("electron");
class PlatformInspectorService {
    container;
    workspaceService;
    logger;
    constructor(container, workspaceService, logger) {
        this.container = container;
        this.workspaceService = workspaceService;
        this.logger = logger;
    }
    generateDiagnostics(manifests = []) {
        ...
        try {
            ...
            // 8. permissions.json
            fs.writeFileSync(path.join(archDir, 'permissions.json'), JSON.stringify(Object.values(shared_1.Permission), null, 2));
            // 9. features.json
            ...
            // 12. ipc-map.json
            const ipcHandlers = electron_1.ipcMain.eventNames().map((name) => String(name));
    ...
    generateRuntimeDiagnostics(kernel) {
        const rootPath = this.workspaceService.getRootPath();
        if (!rootPath)
            return;
...
exports.PlatformInspectorService = PlatformInspectorService;
//# sourceMappingURL=platform-inspector-service.js.map