...
class WindowService {
    ...
    maximize() {
        const w = this.win;
        if (!w) {
            this.logger.warn('[WindowService] maximize(): main window not found');
            return;
        }
        w.maximize();
    }
    minimize() {
        const w = this.win;
        if (!w) {
            this.logger.warn('[WindowService] minimize(): main window not found');
            return;
        }
        w.minimize();
    }
    restore() {
        const w = this.win;
        if (!w) {
            this.logger.warn('[WindowService] restore(): main window not found');
            return;
        }
        if (w.isMaximized())
            w.restore();
    ...
    toggleFullscreen() {
        const w = this.win;
        if (!w) {
            this.logger.warn('[WindowService] toggleFullscreen(): main window not found');
            return;
        }
