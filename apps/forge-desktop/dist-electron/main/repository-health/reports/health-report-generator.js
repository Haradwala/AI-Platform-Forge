...
class HealthReportGenerator {
    ...
    generateReport(snapshot, findings, metrics) {
        ...
        for (const f of findings) {
            ...
            if (f.category === 'architecture')
                ...
                complexityPenalties += mult;
            else if (f.category === 'dead-code')
                deadPenalties += mult;
            else if (f.category === 'duplicate')
                dupPenalties += mult;
            else if (f.category === 'dependency')
                depPenalties += mult;
        }
        ...
        let overallScore = Math.round(archScore * 0.25 +
